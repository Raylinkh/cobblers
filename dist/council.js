import { OpenRouterClient, DEFAULT_MODELS } from "./openrouter.js";
...
export class Council {
    ...
    async deliberate(question) {
        ...
        for (let roundNum = 1; roundNum <= this.config.rounds; roundNum++) {
            const round = { number: roundNum, turns: [] };
    ...
    buildPrompt(question, roundNum, history, persona) {
    ...
    async synthesize(question, rounds) {
...
- What questions need further exploration?`
    ...
    async review(filePath, fileContent) {
...
export function formatResult(result, color = true) {
    const c = {
        reset: color ? "\x1b[0m" : "",
        bold: color ? "\x1b[1m" : "",
        dim: color ? "\x1b[2m" : "",
